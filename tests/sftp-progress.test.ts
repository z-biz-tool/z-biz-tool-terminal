...
const panel = readFileSync("src/components/SftpPanel.tsx", "utf8");
{
  ...
  // 成功只有一个来源：`error === null`（批量那条先分进 outcome.saved，再由汇总/单条快路径决定）
  ok(
    "成功文案必须在 error===null 之后",
    (panel.match(/if \(error === null\) message\.success/g) || []).length === 1 &&
      (panel.match(/if \(error === null\) outcome\.saved\.push\(plan\)/g) || []).length === 2 &&
      /if \(summary\.kind === "success"\) message\.success\(summary\.text\)/.test(panel) &&
      panel.indexOf("const only = outcome.failed[0];") < panel.indexOf("`上传成功:")
  );
