...
| 分屏按钮可区分 | 每一格的关闭键名称带序号与所在主机（`关闭第 1/2 格 · alpha`），读屏不再听到 N 个同名按钮；静态守卫 `button-labels` 同时禁止模板串里出现漏写 `$` 的字面量花括号（真犯过，界面上会印出 `{tab.panes.length}`） |
| 中断与原子落盘 | 传输条上「中断当前」与「取消剩余」是两个入口：前者带当前 `transfer_id` 请求后端在下一块之前停下，后者只挡还没开始的；被用户中断的一条报「已中断」而不是「失败」。上传与下载都改为先写 `名字.<id>.part` 再 rename：取消或半路失败既不会留下冒充完成的半截本地文件（也就不会被「目标已存在」那道闸挡住重试），也不会把远端原文件截断；汇总里说成「分片已丢弃，原文件未被改写」而不是「失败」 |
| 关掉面板即收手 | 关闭 SFTP 面板时剩下没开始的一律不再开始（实测在传那条完成后 `sftp_download` 从 3 次降到 1 次、不再弹任何提示）；进度条收尾定时器与目录刷新同样带存活判断。正在传的那条仍会传完（后端没有中断单条传输的能力） |
...
- 已实测通过：`npm run typecheck` 0 错误、hover/focus 高亮全部改成状态驱动（全仓元素级 `node.style.X = ...` 清零，只剩白名单里的 document 级光标等；React 才是不一致的样式来源）、命令面板的鼠标/键盘分档（实测修前鼠标扫过一行，Enter 执行的就换成那一行；现在 hover 只给弱高亮，Enter 只认键盘指向，点击仍点哪行走哪条）、启动包拆分（16 个只在点开后才存在的面板改走 `lazyPanel`，启动要解析的字节 1,913,304 → 1,791,021 B，见 §7.43；DangerConfirm/HostKeyPrompt 等安全闸明确要求保持启动加载）、`npm test` 9002 例（40 个文件：`palette-order` 6801 例命令面板序列不变量——对 1/2/3/4/6 个条目**穷举全部排列**逐条验幂等、同类连续、首项不被类别推翻，不是重复跑同一个用例、`shortcuts` 128 例快捷键漂移守卫（含 README 表与注册表逐字对账）、`close-guard` 113 例关闭确认闸、`reconnect-progress` 104 例重连进度与退避调度、`settings-sanity` 96 例配置取值闸、`terminal-search` 96 例终端搜索判定与落点几何、`search-staleness` 161 例搜索结果随新输出过期的锚点回找（含 574 例确定性 fuzz 的后条件：认得回来就必须落在同一行文本上，认不回来就必须是 `null`）、`sftp-edit-path` 67 例编辑副本落点（23 行具名矩阵 + 3000 例确定性 fuzz + 与 Rust 落盘闸的跨语言对账）、`confirm-queue` 60 例确认排队契约、`palette-session` 47 例命令面板会话陈述、`sftp-batch` 67 例整批进度、取消与整批字节百分比（2000 轮随机 start/finish/cancel 交错 + 800 批随机大小/实时字节：百分比不越界、结束时必到 100%、任一大小未知则整段不给百分比、对不上文件名的实时值一律忽略）、`toolbar-identity` 22 例顶栏按钮身份（21 颗按钮图标两两不重复、同一动作不画两遍、每颗都有 aria-label 或可见文字；按花括号深度切 JSX 开始标签）、`antd-v6-apis` 18 例 antd v6 废弃属性清零（7 类禁用词命中必须为 0 + 替代名逐个计数 + 两条 P-2 遮罩行为 + List 债名单）、`sftp-progress-speed` 48 例传输用时/均速/约剩（4000 组随机进度 + 时钟，含时钟倒挂：不出 NaN/负号、大小未知不给剩余、任何时长不出「X 分 60 秒」）、`button-labels` 29 例全仓按钮可及名称普查（带图标的 `<Button>` 必须有 aria-label 或可见文字、不许裸 title、用了 Tooltip 就得 import；行内动作要带行名）、`config-import` 31 例导入配置的确认闸与解密态（假 invoke 跑真 store + 跨语言守卫）、`sftp-download-plan` 51 例批量下载落点与汇总（3000 例确定性 fuzz：落点两两不同、不含分隔符/控制字符、不空不超长）、`sftp-session-identity` 84 例 SFTP 面板会话身份与列表归属（`pickTabSession` 取值矩阵 + 旧写法的反面对照 + `listingCandidates` 恒列性质）、`sftp-progress` 365 例 SFTP 传输状态机（10 行认领矩阵、240 轮确定性属性序列、payload 键与 Rust `serde_json::json!` 逐字对账）等，每轮另跑变异对照）、`npm run build`（启动包 1,791.16 kB / gzip 544.26 kB（21 个分片；此前是 1,913 kB 单包））、`cargo fmt --check`（在 `src-tauri/` 下跑：仓库根没有 `Cargo.toml`，在根上跑只会打印 usage 并以 1 退出）、`cargo test --lib` 86 例。
