...
- 已实测通过：`npm run typecheck` 0 错误、`npm test` 8845 例（35 个文件：`palette-order` 6801 例命令面板序列不变量——对 1/2/3/4/6 个条目**穷举全部排列**逐条验幂等、同类连续、首项不被类别推翻，不是重复跑同一个用例、`shortcuts` 128 例快捷键漂移守卫（含 README 表与注册表逐字对账）、`close-guard` 113 例关闭确认闸、`reconnect-progress` 104 例重连进度与退避调度、`settings-sanity` 96 例配置取值闸、`terminal-search` 96 例终端搜索判定与落点几何、`search-staleness` 161 例搜索结果随新输出过期的锚点回找（含 574 例确定性 fuzz 的后条件：认得回来就必须落在同一行文本上，认不回来就必须是 `null`）、`sftp-edit-path` 67 例编辑副本落点（23 行具名矩阵 + 3000 例确定性 fuzz + 与 Rust 落盘闸的跨语言对账）、`confirm-queue` 60 例确认排队契约、`palette-session` 47 例命令面板会话陈述、`sftp-batch` 67 例整批进度、取消与整批字节百分比（2000 轮随机 start/finish/cancel 交错 + 800 批随机大小/实时字节：百分比不越界、结束时必到 100%、任一大小未知则整段不给百分比、对不上文件名的实时值一律忽略）、`toolbar-identity` 22 例顶栏按钮身份（21 颗按钮图标两两不重复、同一动作不画两遍、每颗都有 aria-label 或可见文字；按花括号深度切 JSX 开始标签）、`antd-v6-apis` 18 例 antd v6 废弃属性清零（7 类禁用词命中必须为 0 + 替代名逐个计数 + 两条 P-2 遮罩行为 + List 债名单）、`sftp-progress-speed` 48 例传输用时/均速/约剩（4000 组随机进度 + 时钟，含时钟倒挂：不出 NaN/负号、大小未知不给剩余、任何时长不出「X 分 60 秒」）、`config-import` 31 例导入配置的确认闸与解密态（假 invoke 跑真 store + 跨语言守卫）、`sftp-download-plan` 51 例批量下载落点与汇总（3000 例确定性 fuzz：落点两两不同、不含分隔符/控制字符、不空不超长）、`sftp-session-identity` 84 例 SFTP 面板会话身份与列表归属（`pickTabSession` 取值矩阵 + 旧写法的反面对照 + `listingCandidates` 恒列性质）、`sftp-progress` 365 例 SFTP 传输状态机（10 行认领矩阵、240 轮确定性属性序列、payload 键与 Rust `serde_json::json!` 逐字对账）等，每轮另跑变异对照）、`npm run build`（单 chunk 1,919.52 kB / gzip 579.99 kB）、`cargo fmt --check`（在 `src-tauri/` 下跑：仓库根没有 `Cargo.toml`，在根上跑只会打印 usage 并以 1 退出）、`cargo test --lib` 82 例。
