...
| 中断与原子落盘 | 传输条上「中断当前」与「取消剩余」是两个入口：前者带当前 `transfer_id` 请求后端在下一块之前停下，后者只挡还没开始的；被用户中断的一条报「已中断」而不是「失败」。下载改为先写同目录 `名字.<id>.part` 再 rename —— 取消或半路失败不会留下冒充完成的半截文件，也就不会反过来被「目标已存在」那道闸挡住重试 |
